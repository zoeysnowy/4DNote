...
import { EventNode, CreateEventNodeInput, UpdateEventNodeInput, QueryEventNodesInput } from '@frontend/types/EventNode';
  ...
  private static convertTimestampToTimeSpec(timestamp: number | string): string {
    if (typeof timestamp === 'number') {
      ...
      console.log('[EventNodeService] 转换时间戳:', {
        原始值: timestamp,
        类型: 'number',
        Date对象: formatTimeForStorage(new Date(timestamp)),
        转换后: converted
      });
      return converted;
    }
