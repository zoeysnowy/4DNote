param(
  ...
  [Parameter(Mandatory = $false)]
  [string]$LogsPath = ".vscode\\_oom_logs_capture",
...
function Write-Info([string]$message) {
  Write-Host "[prune-oom-logs] $message"
}

$root = Resolve-Path -LiteralPath $LogsPath -ErrorAction SilentlyContinue
if (-not $root) {
  Write-Info "No logs folder found at '$LogsPath'. Nothing to do."
  exit 0
}
