...
(function() {
  ...
  async function testEventHub() {
    ...
    try {
      ...
      await assert(
        event.content[0].children[0].text === '通过 EventHub 更新的内容',
        'EventHub.updateContent() 成功',
        { content: event.content }
      );
    } catch (error) {
      await assert(false, 'EventHub.updateContent() 失败', { error: error.message });
    }
    ...
    try {
      ...
      const event = await window.EventService.getEventById(testEventId);
      await assert(
        event.tags.includes('测试标签'),
        'EventHub.addTag() 成功',
        { tags: event.tags }
      );
    } catch (error) {
      await assert(false, 'EventHub.addTag() 失败', { error: error.message });
    }
    ...
    try {
      ...
      await assert(false, 'EventHub.removeTag() 失败', { error: error.message });
    }
