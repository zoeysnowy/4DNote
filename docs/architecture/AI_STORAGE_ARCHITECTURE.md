...
> **更新时间**: 2025-12-15  
> **状态**: 📋 规划中  
> **核心理念**: 混合架构 - 桌面端本地优先 AI + 移动端云端优先 AI  
> **技术栈**: 
> - **桌面端**: Dexie.js + FlexSearch + Transformers.js + Voyager
> - **移动端**: React Native + AI Gateway + Cloud AI APIs
...
- [4. 移动端云端优先架构](#4-移动端云端优先架构)
- [5. 混合搜索引擎架构（桌面端）](#5-混合搜索引擎架构桌面端)
- [6. 核心服务模块](#6-核心服务模块)
- [7. 性能优化策略](#7-性能优化策略)
- [8. 实施路线图](#8-实施路线图)
- [9. 技术决策记录](#9-技术决策记录)
...
构建一个**跨平台智能笔记应用**，在桌面端和移动端采用不同但互补的 AI 策略：

1. **原子化数据 (Atomic Data)**：万物皆 Event，打破传统文档/文件夹边界
2. **平台化 AI 架构**：
   - **桌面端**：本地优先 (Local-First) - 隐私保护 + 离线可用
   - **移动端**：云端优先 (Cloud-First) - 省电省流量 + 性能优化
...
| 维度 | 桌面端 | 移动端 | 共享部分 |
|------|--------|--------|----------|
| **数据模型** | UniversalEvent | UniversalEvent | ✅ 完全相同 |
| **主数据存储** | IndexedDB (本地) | Cloud DB (云端) | 🔄 双向同步 |
| **AI 推理位置** | Web Worker (浏览器) | AI Gateway (云端) | ❌ 平台专属 |
| **搜索引擎** | FlexSearch + Voyager | Cloud API | ❌ 平台专属 |
| **离线能力** | ✅ 完全离线可用 | ⚠️ 缓存数据只读 | - |
| **隐私保护** | ✅ 数据不离开设备 | ⚠️ 依赖服务端加密 | - |
...
> **关键决策**：桌面端和移动端采用不同的 AI 架构策略

```
┌──────────────────────────────────────────────────────────────────┐
│                    平台 AI 策略对比                               │
├──────────────────┬────────────────────┬──────────────────────────┤
│ 对比维度         │ 桌面端 (Desktop)   │ 移动端 (Mobile)          │
├──────────────────┼────────────────────┼──────────────────────────┤
│ **核心策略**     │ 本地优先 (Local)   │ 云端优先 (Cloud)         │
│ **AI 推理位置**  │ 浏览器 Web Worker  │ 云端 API Gateway         │
│ **向量检索位置** │ 本地 IndexedDB     │ 云端向量数据库           │
│ **数据存储位置** │ IndexedDB (主)     │ 云端 (主) + 本地缓存     │
├──────────────────┼────────────────────┼──────────────────────────┤
│ **优势**         │ • 隐私保护         │ • 省电省流量             │
│                  │ • 离线可用         │ • 性能更好               │
│                  │ • 无 API 成本      │ • 模型更新灵活           │
...
**决策原因**：
- **桌面端**：浏览器环境资源充足，WebAssembly 性能可接受，用户期望离线可用
- **移动端**：电池/内存受限，本地 AI 推理耗电严重，云端处理体验更好

### 2.2 技术栈规范

#### 2.2.1 桌面端技术栈 (Desktop - Local-First)
...
#### 2.2.2 移动端技术栈 (Mobile - Cloud-First)

| 技术层 | 选型 | 版本 | 理由 |
|--------|------|------|------|
| **语言** | TypeScript | 5.x | 与桌面端共享类型定义 |
| **框架** | React Native | 0.74+ | 跨平台，代码复用率高 |
| **状态管理** | Zustand | 4.x | 与桌面端一致 |
| **本地存储** | AsyncStorage | - | 轻量缓存（非主数据源） |
| **云端 API** | AI Gateway | 自研 | 统一 AI 服务入口 |
| **AI 服务** | GPT-4 / Claude | API | 文本生成与对话 |
| **向量检索** | Jina AI Rerank | API | 云端向量重排序 |
| **OCR 服务** | Google Vision | API | 图片文字识别 |
| **富文本** | Slate.js (简化版) | 0.100+ | 移动端适配的轻量编辑器 |

### 2.3 架构四大支柱

#### 2.3.1 桌面端架构 (Local-First)
...
├────────────────────────────────────────────────────────────┤
│                                                            │
│  1️⃣ 数据分层          2️⃣ AI Gateway        3️⃣ 时间聚合   │
│  ┌─────────┐          ┌─────────┐         ┌─────────┐    │
│  │ 云端主库 │          │ GPT-4   │         │Timeline │    │
│  │ (主存储) │  <───>  │ Claude  │  <───>  │View     │    │
│  └─────────┘          └─────────┘         └─────────┘    │
...
graph TB
    subgraph "桌面端用户"
        Desktop[浏览器客户端]
        LocalDB[(IndexedDB)]
        WebWorker[AI Worker<br/>Transformers.js]
    end
    
    subgraph "移动端用户"
        Mobile[React Native App]
        Cache[(AsyncStorage<br/>缓存)]
        BGQueue[后台任务队列]
    end
    
    subgraph "云端服务"
        CloudDB[(主数据库<br/>PostgreSQL)]
        AIGateway[AI Gateway]
        GPT[GPT-4 API]
        JinaAI[Jina AI Rerank]
    end
    ...
    style CloudDB fill:#e8f5e9
    style AIGateway fill:#f3e5f5
```

**关键设计决策**：
- **数据同步**：桌面端和移动端共享同一云端数据库，确保跨设备数据一致性
- **AI 推理**：桌面端本地处理，移动端云端处理，各取所长
- **降级策略**：移动端网络中断时，使用缓存数据 + 本地搜索（关键词匹配），不执行 AI 功能
...
│  │  AI Gateway (核心)                                     │ │
│  │  ┌──────────────┐  ┌──────────────┐                  │ │
│  │  │ GPT-4/Claude │  │ Jina AI      │                  │ │
│  │  │ 文本生成     │  │ Rerank       │                  │ │
│  │  └──────────────┘  └──────────────┘                  │ │
│  │  ┌──────────────┐  ┌──────────────┐                  │ │
│  │  │ Google Vision│  │ Embedding    │                  │ │
│  │  │ OCR 识别     │  │ 向量化       │                  │ │
│  │  └──────────────┘  └──────────────┘                  │ │
│  └────────┬───────────────────────────────────────────────┘ │
│           ↓                                                  │
...
```typescript
/**
 * AI Gateway - 移动端 AI 服务统一入口
 * 
 * 核心职责：
 * 1. 统一 AI API 调用（GPT-4, Claude, Jina AI, Google Vision）
 * 2. 请求批处理（降低 API 调用成本）
 * 3. 结果缓存（减少重复计算）
 * 4. 降级策略（API 失败时使用简单规则）
 * 5. 成本控制（按用户/设备限流）
 */
 ...
 * 批处理优化 - 降低 API 调用成本
 * 
 * 场景：用户快速滑动列表，触发 10 个 OCR 任务
 * 问题：单独调用 Google Vision API 10 次 → $0.15
 * 优化：批量调用 1 次 → $0.015 (节省 90%)
 */
...
class BatchProcessor {
  ...
  private async flush() {
    ...
    const batch = this.queue.splice(0, 10); // 最多批量处理 10 个
    const results = await this.callBatchAPI(batch);
 ...
 * 多级缓存 - 减少重复 AI 计算
 * 
 * L1: 内存缓存 (Redis) - 热数据 1 小时
 * L2: 持久化缓存 (PostgreSQL) - 向量/OCR 结果永久保存
 * L3: 客户端缓存 (AsyncStorage) - 离线可用
 */
...
class CacheManager {
  async getOrCompute(
    ...
    // L2: 检查持久化缓存
    const persisted = await db.cache.findOne({ key });
    if (persisted) {
      await redis.set(key, JSON.stringify(persisted.value), { EX: ttl });
    ...
    await db.cache.create({ key, value: result });
 ...
 * - 后台队列延迟 5 秒同步到云端
 * 
 * 场景 2: 用户发起 ChatFlow 查询
 * - 优先使用本地缓存数据
 * - 后台静默同步最新数据
 * 
 * 场景 3: 用户上传图片 OCR
 * - WiFi 环境：立即上传处理
 * - 移动网络：加入后台队列，WiFi 时处理
 */

class SyncManager {
  async syncEvent(event: Event, priority: 'high' | 'low' = 'low') {
    // 1. 立即更新本地缓存
    ...
    if (priority === 'high' || networkState.type === 'wifi') {
      // 高优先级或 WiFi 环境：立即同步
      await this.uploadToCloud(event);
    } else {
      // 低优先级 + 移动网络：加入后台队列
...
```typescript
/**
 * 冲突解决 - Last-Write-Wins + 事件历史记录
 * 
 * 场景：用户在桌面端和移动端同时编辑同一事件
 * 策略：以 updated_at 时间戳为准，后写入者胜出
 */
...
async function resolveConflict(
  ...
  if (localEvent.updated_at > cloudEvent.updated_at) {
    // 本地版本更新 → 上传覆盖云端
    await cloudAPI.updateEvent(localEvent);
    return localEvent;
  } else {
    // 云端版本更新 → 下载覆盖本地
 ...
 * 
 * 策略：
 * 1. 免费用户：每月 100 次 AI 调用（ChatFlow 50 次 + OCR 50 次）
 * 2. 付费用户：每月 10000 次 AI 调用
 * 3. 超限后：降级到本地关键词搜索
 */

class QuotaManager {
  async checkQuota(userId: string, action: string): Promise<boolean> {
    const usage = await redis.get(`quota:${userId}:${action}`);
    const limit = await this.getUserLimit(userId, action);
    
    if (parseInt(usage || '0') >= limit) {
      // 超限 → 降级策略
      logger.warn(`User ${userId} exceeded quota for ${action}`);
      return false;
    }
    
    // 计数器 +1
    await redis.incr(`quota:${userId}:${action}`);
    return true;
  }
}
```
 ...
 * 
 * 策略：
 * 1. 图片压缩：OCR 前压缩到 1024x1024，JPEG 质量 80%
 * 2. 增量同步：仅传输 changed fields，不传整个 event
 * 3. Gzip 压缩：API 请求/响应启用 gzip
 */

class BandwidthOptimizer {
  async compressImage(uri: string): Promise<Blob> {
    const manipulated = await ImageManipulator.manipulateAsync(
      uri,
      [{ resize: { width: 1024 } }],
      { compress: 0.8, format: SaveFormat.JPEG }
    );
    return manipulated.blob;
  }
  ...
  async incrementalSync(
    ...
    // 计算 diff，仅传输变更字段
    const diff: Partial<Event> = {};
    for (const key in localEvent) {
      if (localEvent[key] !== cloudEvent[key]) {
        diff[key] = localEvent[key];
      }
    }
    return diff;
  }
...
### 5.3 引擎 B：语义引擎（Transformers.js + Voyager）
...
### 9.1 为什么选择 Dexie.js 而非原生 IndexedDB？
...
### 9.2 桌面端为什么选择 Transformers.js 而非云端 API？

**决策**：桌面端使用本地 AI 模型（Transformers.js），移动端使用云端 API

**理由**：

**桌面端选择本地 AI**：
1. ✅ **隐私保护**：数据不离开设备，适合企业/个人隐私需求
2. ✅ **离线可用**：无网络依赖，飞行模式也能搜索
3. ✅ **成本控制**：零 API 调用费用，适合高频查询场景
4. ✅ **响应速度**：本地推理 <500ms，无网络延迟

**移动端选择云端 API**：
1. ✅ **省电**：避免本地 AI 模型推理导致的严重耗电
2. ✅ **性能**：云端 GPU 推理速度远超移动端 CPU
3. ✅ **内存占用低**：移动端内存紧张，云端计算无压力
4. ✅ **模型更新灵活**：云端更新模型，客户端无需下载

**权衡对比**：

| 指标 | 桌面端本地 AI | 移动端云端 API |
|------|--------------|---------------|
| 首次加载 | 2-5 秒 (模型下载) | 0 秒 |
| 推理速度 | 200-500ms | 100-300ms (含网络) |
| 电池消耗 | ⚠️ 中等 (浏览器可接受) | ✅ 低 (云端计算) |
| 内存占用 | ~23MB (浏览器可承受) | < 5MB (仅缓存数据) |
| 隐私性 | ⭐⭐⭐⭐⭐ | ⭐⭐ (依赖服务端加密) |
| 离线可用 | ✅ 完全可用 | ⚠️ 降级到关键词搜索 |
| 成本 | $0 | $0.0001/次 (可控) |

**结论**：根据平台特性选择合适的 AI 策略，桌面端优先隐私和离线，移动端优先性能和续航。

---

### 9.3 为什么使用 created_at 而非 updated_at 作为时间轴排序？
...
**决策**：移动端 AI 推理和向量检索全部在云端，桌面端保持本地优先
...
| **隐私保护** | ✅ 数据不离开设备 | ⚠️ 需服务端加密保障 |
| **API 成本** | ✅ 零成本 | ⚠️ 按调用量计费 (可控) |

**关键技术考量**：
1. **移动端资源受限**：
   - 本地 AI 模型推理在移动端会导致**严重耗电**和**卡顿**
   - React Native 的 JS 引擎性能远低于桌面浏览器的 V8
   - iOS 对后台进程有严格限制，本地 AI 难以在后台运行

2. **用户体验优先**：
   - 移动用户期望**快速响应**，云端 GPU 推理速度 > 本地 CPU
   - WiFi 环境下，网络延迟可忽略（< 100ms）
   - 移动网络下，可降级到本地关键词搜索

3. **成本可控**：
   - 通过**请求批处理**降低 API 调用次数（节省 90%）
   - 通过**多级缓存**避免重复计算（命中率 > 70%）
   - 免费用户每月 100 次调用，付费用户 10000 次

**降级策略**：
```typescript
// 网络中断时的降级方案
if (!isOnline) {
  // 使用本地缓存 + 关键词匹配（不执行 AI 功能）
  return await localCache.searchByKeyword(query);
} else if (quota.exceeded) {
  // API 配额耗尽时降级
  return await localCache.searchByKeyword(query);
} else {
  // 正常调用云端 AI
  return await cloudAI.semanticSearch(query);
}
```

**结论**：
- **桌面端本地优先**：适合注重隐私、离线使用的场景
- **移动端云端优先**：适合注重性能、电池续航的场景
- **平台各取所长**：数据模型统一，AI 策略灵活适配
