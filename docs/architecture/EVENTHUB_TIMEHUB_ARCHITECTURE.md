...
## 8. EventService 重构优化（v2.18.8）

### 8.1 normalizeEventLog 组件化重构
...
- 🚀 **Diff 算法优化**：O(n) → O(1)
  ```typescript
  // 优化前：遍历所有节点比较（100个节点 = 100次比较）
  for (let i = 0; i < newTexts.length; i++) {
    if (newTexts[i] !== oldTexts[i]) { /* ... */ }
  }
  
  // 优化后：只检查新增 + 最后一个节点（常数时间）
  if (newTexts.length > oldTexts.length) {
    // 只处理新增节点
  } else if (newTexts[lastIndex] !== oldTexts[lastIndex]) {
    // 只检查最后一个
  }
  ```
  - 性能提升：100个节点时 ~100倍
...
- 🗑️ **移除 hasChanges 返回值**
  - updateEvent 中用 JSON.stringify 做最终验证
  - 简化代码，减少状态传递
