...
优化方案：
查询 1000 个 EventStats 对象 → 预计 < 200ms
数据量：~50KB
性能提升：5倍+ 🚀
```
...
async updateEvent(id: string, updates: Partial<Event>): Promise<Event> {
  ...
  if (Object.keys(statsUpdates).length > 0) {
    ...
    await storageManager.updateEventStats(id, statsUpdates);
  }
...
async deleteEvent(id: string): Promise<void> {
  ...
  await storageManager.deleteEventStats(id); // 同步删除
}
```
...
async migrateToEventStats(): Promise<void> {
  ...
  console.log('[StorageManager] 🔄 开始迁移 event_stats...');
  ...
  await this.indexedDBService.bulkCreate('event_stats', statsRecords);
...
1. ✅ 修改 `IndexedDBService.ts` - 添加 `event_stats` objectStore
2. ✅ 更新 DB_VERSION（触发 onupgradeneeded）
3. ✅ 定义 `EventStats` interface

**Phase 2: CRUD 同步**（1小时）
1. ✅ StorageManager 新增 `createEventStats()`, `updateEventStats()`, `deleteEventStats()`
2. ✅ EventService 修改 CRUD 方法，双写 Event + EventStats
3. ✅ 添加 `getEventStatsByDateRange()` 查询方法

**Phase 3: StatsPanel 优化**（30分钟）
1. ✅ 修改 `StatsPanel.tsx` - 使用 `getEventStatsByDateRange()`
2. ✅ 移除不必要的字段访问
3. ✅ 性能测试验证
...
2. **渐进式升级**：`event_stats` 不存在时降级到查询完整 Event
3. **数据一致性**：CRUD 双写保证 Event ↔ EventStats 同步
