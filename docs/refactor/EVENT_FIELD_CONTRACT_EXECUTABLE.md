...
2. **All-Day Event（全天事件）**
   - **判定**：`isAllDay === true` 或 `(startTime && endTime && 同日期 && 都是00:00)`
   - **位置**：日历顶部全天事件栏（横条显示）
   - **示例**：节假日、生日、全天会议
...
3. **Task Bar（任务栏）**
   ...
   - **解释**：
     ...
     - **但没有完整时间段**（`!startTime || !endTime`），所以无法在时间轴上渲染时间块
...
```
1. if (isAllDay || 同日00:00) → All-Day Event
