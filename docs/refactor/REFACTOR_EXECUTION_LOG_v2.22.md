...
| 2026-01-04 | P3-3: move shared UI into `components/shared` | Match target tree by moving shared UI components into `src/components/shared` and migrating imports to `@frontend/components/shared/*` | Low | `npx tsc --noEmit`; `npm test -- --run` | 93b6df3 | Moved: `StatusLineContainer.*`, `SyncNotification.*` → `src/components/shared/*`; updated imports in App + PlanManager |
| 2026-01-04 | P3-4: move shared UI into `components/shared` (GlassIconBar + UnifiedMentionMenu) | Continue matching target tree by moving remaining shared UI components into `src/components/shared` and migrating imports to `@frontend/components/shared/*` and `@backend/*` | Low | `npx tsc --noEmit`; `npm test -- --run` | 1d81e27 | Moved: `GlassIconBar.*`, `UnifiedMentionMenu.*` → `src/components/shared/*`; updated imports in TimeLog pages + PlanSlate/ModalSlate; fixed moved component service import to `@backend/search/UnifiedSearchIndex` |
| 2026-01-04 | P3-5: align `components/attachments` | Match target tree by moving attachment-related UI components under `src/components/attachments` and migrating imports to `@frontend/*` + `@backend/*` aliases (no relative paths) | Low | `npx tsc --noEmit`; `npm test -- --run` | 314d340 | Moved: `AttachmentUploader`, `AttachmentViewContainer`, `AttachmentViewModeSwitcher`, `GalleryView`, `VideoStreamView`, `AudioStreamView`, `TranscriptView`, `DocumentLibView`, `TreeNavigationView`, `BookmarkView` → `src/components/attachments/*`; fixed `AttachmentService` imports and updated `src/components/attachments/index.ts` exports |
| 2026-01-04 | P3-6: move FloatingToolbar into `components/shared` | Match target tree by moving FloatingToolbar under `src/components/shared/FloatingToolbar` and migrating all imports to `@frontend/*` + `@backend/*` aliases (no relative paths) | Low | `npx tsc --noEmit`; `npm test -- --run` | ee8a00b | Moved: `src/components/FloatingToolbar/*` → `src/components/shared/FloatingToolbar/*`; updated consumers (Event/Plan/TimeLog) and internal imports (icons/utils/hooks/services) to alias-only |
| 2026-01-04 | P3-7: move demo pages into `components/demos` | Match target tree by moving demo/test components under `src/components/demos` and migrating imports (including dynamic imports) to `@frontend/*` + `@backend/*` aliases | Low | `npx tsc --noEmit`; `npm test -- --run` | ad439ac | Moved: `AIDemo.*`, `AIDemoV2.*`, `RAGDemo.*` → `src/components/demos/*`; updated App lazy imports and internal demo imports to alias-only |
| 2026-01-04 | P0 cleanup: remove remaining TimerCard shim | Enforce no-shim policy by deleting the unused `src/components/TimerCard.tsx` re-export shim | Low | `npx tsc --noEmit`; `npm test -- --run` | d5b22aa | Deleted: `src/components/TimerCard.tsx` (re-exported `features/Timer`) |
| 2026-01-04 | P3-8: extract Outlook HTML pipeline helpers | Continue P3 by extracting Outlook HTML normalization helpers out of EventService into `services/eventlogProcessing` (no behavior change) | Low | `npx tsc --noEmit`; `npm test -- --run` | ba0c853 | Added `src/services/eventlogProcessing/outlookHtmlPipeline.ts`; EventService now calls `cleanOutlookXmlTags/processMsoLists/sanitizeInlineStyles` |
| 2026-01-04 | P3-9: extract Outlook HTML deep normalization helpers | Continue P3 by extracting the remaining Outlook HTML cleanup steps (signature removal, recursive entity decoding, HTML→plainText extraction) out of EventService into `services/eventlogProcessing` | Low | `npx tsc --noEmit`; `npm test -- --run` | f107898 | Added `removeOutlookSignatureFromHtml/decodeHtmlEntitiesRecursively/extractPlainTextPreservingBreaks`; EventService uses `@backend/eventlogProcessing/outlookHtmlPipeline` |
| 2026-01-06 | EventTree: delete legacy `childEventIds` field (ADR-001 only) | Remove remaining runtime reads/writes of a stale, non-authoritative structural field; enforce single-truth (`parentEventId + position`) | Med | `npm run build` | d94425a | **Field**: `childEventIds`\n**Completion**: runtime src = 0 files (fully removed); tooling still references: scripts = 3 files / 30 matches, public diagnostics = 9 files / 40 matches (kept for legacy inspection).\n**Replacements**: children/siblings derived by querying `parentEventId` (EventService.getChildEvents + EventTreeAPI.getDirectChildren/getSubtree), edges built from `child.parentEventId`.\n**Touched modules**: EventService tree helpers, EventTree UI (Canvas/Slate/Summary/Edit), EventEditModal relation summary, SSOT/README wording. |
| 2026-01-04 | P3-10: align `state/` stores | Complete the P3 store move by keeping global stores under `src/state` and enforcing alias-only imports for store consumers | Low | `npx tsc --noEmit`; `npm test -- --run` | bf1f594 | App now imports `AuthStore/SyncStatusStore` via `@frontend/state/*`; added `src/state/uiStore.ts`; `SyncStatusStore` imports `@frontend/utils/timeUtils` |
| 2026-01-04 | P3-11: services alias-only imports (slice 1) | Start repo-wide alias-only cleanup by converting cross-directory relative imports inside core services to `@frontend/*` + `@backend/*` | Low | `npx tsc --noEmit`; `npm test -- --run` | 6e2a052 | Updated imports in EventService, ActionBasedSyncManager, EventHub, Contact/Tag/Calendar/TimeHub, UnifiedSearchIndex, and EventService service tests |
...
| 2026-01-04 | P3-22: ContactPicker alias-only imports | Convert cross-directory relative imports in `src/components/common/ContactPicker.tsx` to `@frontend/*` + `@backend/*` aliases | Low | `npx tsc --noEmit`; `npm test -- --run` | 2d2c50e | Import-only changes; no behavior change |
| 2026-01-04 | P3-23: ContactPreviewCard alias-only imports | Convert cross-directory relative imports in `src/components/common/ContactPreviewCard.tsx` to `@frontend/*` + `@backend/*` aliases | Low | `npx tsc --noEmit`; `npm test -- --run` | 42d2717 | Import-only changes; no behavior change |
...
| 2026-01-04 | P3-28: TimeCalendar alias-only imports | Convert cross-directory relative imports (including dynamic imports and CSS) in `src/features/Calendar/TimeCalendar.tsx` to `@frontend/*` + `@backend/*` aliases | Low | `npx tsc --noEmit`; `npm test -- --run` | af4372c | Import-only changes; no behavior change |
| 2026-01-04 | P3-29: CalendarGroupManager alias-only imports | Convert cross-directory relative imports in `src/features/Calendar/components/CalendarGroupManager.tsx` to `@backend/*` alias | Low | `npx tsc --noEmit`; `npm test -- --run` | 866f441 | Import-only changes; no behavior change |
...
| 2026-01-04 | P3-37: BaseAgent alias-only imports | Convert cross-directory relative imports in `src/ai/agents/base/Agent.ts` to `@frontend/*` aliases | Low | `npx tsc --noEmit`; `npm test -- --run` | cf11c66 | Import-only changes; no behavior change |
| 2026-01-04 | P3-38: Memory alias-only imports | Convert cross-directory relative imports in `src/ai/agents/base/Memory.ts` to `@frontend/*` aliases | Low | `npx tsc --noEmit`; `npm test -- --run` | 7841390 | Import-only changes; no behavior change |
| 2026-01-04 | P3-39: ai index alias-only exports | Convert relative type/config re-exports in `src/ai/index.ts` to `@frontend/*` aliases | Low | `npx tsc --noEmit`; `npm test -- --run` | fa5ad61 | Import-only changes; no behavior change |
| 2026-01-04 | P3-40: VectorStoreService alias-only imports | Convert type import in `src/ai/services/VectorStoreService.ts` to `@frontend/*` alias | Low | `npx tsc --noEmit`; `npm test -- --run` | 25573b9 | Import-only changes; no behavior change |
| 2026-01-04 | P3-41: BaseTool alias-only imports | Convert tool types import in `src/ai/tools/base/Tool.ts` to `@frontend/*` alias | Low | `npx tsc --noEmit`; `npm test -- --run` | 87cdbf3 | Import-only changes; no behavior change |
| 2026-01-04 | P3-42: OCRTool alias-only imports | Convert BaseTool import in `src/ai/tools/ocr/OCRTool.ts` to `@frontend/*` alias | Low | `npx tsc --noEmit`; `npm test -- --run` | 5b6876a | Import-only changes; no behavior change |
| 2026-01-04 | P3-43: QRCodeTool alias-only imports | Convert cross-directory imports in `src/ai/tools/qrcode/QRCodeTool.ts` to `@frontend/*` aliases | Low | `npx tsc --noEmit`; `npm test -- --run` | d89e1f9 | Import-only changes; no behavior change |
| 2026-01-04 | P3-44: EventExtractionWorkflow alias-only imports | Convert imports (incl. dynamic import) in `src/ai/workflows/EventExtractionWorkflow.ts` to `@frontend/*` aliases | Low | `npx tsc --noEmit`; `npm test -- --run` | 5e721ef | Import-only changes; no behavior change |
