...
| 2026-01-04 | P2-5: move EventEditModalV2 into `features/Event` (no shims) | Continue Event feature boundary per target tree; remove legacy `src/components/EventEditModal` location | Low | `npx tsc --noEmit`; `npm test -- --run` | (local) | Moved: `src/components/EventEditModal/*` → `src/features/Event/components/EventEditModal/*`; updated imports in App/TimeCalendar/Plan/TimeLog/EditorWindow; added `src/features/Event/index.ts` |
