...
| 2026-01-04 | P3-2: move common UI into `components/common` | Match target tree by moving common UI components into `src/components/common` and migrating imports to `@frontend/components/common/*` | Low | `npx tsc --noEmit`; `npm test -- --run` | (local) | Moved: `Logo.*`, `ColorPicker.tsx`, `ErrorBoundary.tsx` → `src/components/common/*`; updated TagManager ColorPicker import |
