...
| 2026-01-04 | P2-3: move ContactModal into `features/Contact` (no shims) | Establish Contact feature boundary per target tree; remove legacy `src/components/ContactModal` location | Low | `npx tsc --noEmit`; `npm test -- --run` | (pending) | Moved: ContactModal + CSS + tests → `src/features/Contact/*`; updated imports to `@frontend/*` + `@backend/*` |
