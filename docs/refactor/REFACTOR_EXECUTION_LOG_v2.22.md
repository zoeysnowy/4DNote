...
| 2026-01-04 | P3-5: align `components/attachments` | Match target tree by moving attachment-related UI components under `src/components/attachments` and migrating imports to `@frontend/*` + `@backend/*` aliases (no relative paths) | Low | `npx tsc --noEmit`; `npm test -- --run` | (commit) | Moved: `AttachmentUploader`, `AttachmentViewContainer`, `AttachmentViewModeSwitcher`, `GalleryView`, `VideoStreamView`, `AudioStreamView`, `TranscriptView`, `DocumentLibView`, `TreeNavigationView`, `BookmarkView` → `src/components/attachments/*`; fixed `AttachmentService` imports and updated `src/components/attachments/index.ts` exports |
