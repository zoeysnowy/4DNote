...
| 2026-01-03 | Plan: move PlanManager into feature folder (shim) | Start codebase “feature boundary” cleanup without breaking existing imports | Low | `npm run typecheck`; `npm run test -- --run` | (local) | New home: `src/features/Plan/components/PlanManager.tsx`; compat shim: `src/components/PlanManager.tsx` |
