...
| 2026-01-04 | P2-2: move Tag UI into `features/Tag` (no shims) | Establish Tag feature boundary per target tree; remove legacy `src/components/TagManager` + `src/components/HierarchicalTagPicker` locations | Low | `npx tsc --noEmit`; `npm test -- --run` | (pending) | Moved: `TagManager` + CSS + `HierarchicalTagPicker/*` → `src/features/Tag/*`; updated imports in Tag page + Event/TimeLog/EditModal |
